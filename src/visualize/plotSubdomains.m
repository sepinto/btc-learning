function [out] = plotSubdomains(data, labels, domain, cdf, probEdges, domainEdges, fignums)
    ...
    title('Numerical CDF of Fitted Distribution', 'fontsize', 30, 'fontweight','normal')
    xlabel('Hours','fontsize',25)
    set(gca,'fontsize',30)
    axis([0 15000 0 1])
    ...
    axis([0 300 0 1])
    xlabel('Hours','fontsize',25)
    title('Equal Probability Subdomains on CDF', 'fontsize', 30, 'fontweight','normal')
    set(gca,'fontsize',30)
